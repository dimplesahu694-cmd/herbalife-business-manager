...
export default function HerbalifeApp() {
  ...
  const addLead = () => {
    ...
    setFormData(prev => ({...prev, leadName: '', leadPhone: '', leadSource: '', leadInterest: '', leadStatus: ''}));
  };
  ...
  const addLog = () => {
    ...
    setFormData(prev => ({...prev, logDate: new Date().toISOString().split('T')[0], logContacts: '', logMessages: '', logResponses: '', logCustomers: ''}));
  };
  ...
  const generateAITasks = async () => {
    ...
    try {
      ...
      setAiTasks('Error generating tasks');
    }
    setAiLoading(false);
  };
  ...
  const styles = {
    ...
    button: { background: '#2563eb', color: '#fff', padding: '10px 16px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' },
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <div><div style={styles.title}>🎯 Herbalife Business Manager</div></div>
        <div><button onClick={handleLogout} style={{...styles.button, background: '#dc2626'}}>Logout</button></div>
      </div>
        ...
        {activeTab === 'dashboard' && (
          <div>
            ...
            <div style={styles.card}>
              ...
              {aiTasks && <div style={{ marginTop: '1rem', padding: '1rem', background: '#f0f9ff', borderRadius: '6px', fontSize: '13px', whiteSpace: 'pre-wrap', lineHeight: '1.6' }}>{aiTasks}</div>}
            </div>
          </div>
        )}
